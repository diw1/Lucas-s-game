import {talents,talentHit,bossIntent,resolveBossAction} from './combat.js';
import {regions,regionAt,chests,quests,openChest,claimQuest} from './adventure.js';
import {SAVE_KEY,readSave,writeSave} from './save.js';
import {completeBoss,evolutionCost,evolve,gainXP,companionName,bossAt,bossCreature,typeFactor,species,collectibleIds,bossTeam,makeCreature,attack,catchChance,tryCapture,touching,nextBossRound,strengths,directions,movementStep} from './rules.js';
import {renderer,portrait} from './render.js';
import {createWorld} from './world.js';
import {bossSpecialEffect,strike,healEffect,captureEffect,switchEffect} from './effects.js';
import {chooseCue,drawCueRing,drawCueBrackets} from './interaction.js';
...
function fighter(c,title,side){return `<div class="fighter" data-side="${side}"><span class="type">${title} · ${species[c.id].type}</span><canvas data-id="${c.id}" data-stage="${c.stage||0}" data-rare="${!!c.rare}" aria-label="${name(c)}"></canvas><h2>${name(c)}</h2><div class="hp"><i style="width:${c.hp/c.max*100}%"></i></div><small class="hp-value">${c.hp} / ${c.max} HP</small></div>`;}
function renderBattle(){
  ...
  panel(`<div class="battle-head"><div><span class="eyebrow">${b.trainer?`BOSS · OPPONENT ${b.round+1} OF ${bossTeam.length}`:'WILD ENCOUNTER'}</span><h1>${b.trainer?`Boss ${b.boss.rank} · ${b.boss.name}`:'Make a new friend'}</h1></div><span class="muted">Turn ${b.turn+1}</span></div>${b.trainer?`<div class="boss-roster" aria-label="Boss team">${b.boss.team.map((id,i)=>`<span class="${i<b.round?'defeated':i===b.round?'current':''}">${i<b.round?'✓ ':''}${species[id].name}</span>`).join('')}</div>`:''}<div class="arena">${fighter(a,'Your companion','player')}<span class="versus">VS</span>${fighter(b.enemy,b.trainer?'Fern’s companion':'Wild companion','enemy')}</div>${b.trainer?`<div class="intent"><strong>Next: ${bossIntent(b).name}</strong><br>${bossIntent(b).detail}${b.enemyShield?' Shield active!':''}</div>`:''}<div class="log" role="status" aria-live="polite">${b.log}</div><div class="moves">${a.stage?`<button data-move="talent" ${(b.talentUses[state.active]||0)>=a.stage+1?'disabled':''}>✦ ${talents[species[a.id].type].name}<small>${talents[species[a.id].type].detail} · ${Math.max(0,a.stage+1-(b.talentUses[state.active]||0))} uses left</small></button>`:''}<button data-move="skill">${species[a.id].move}<small>${species[a.id].type} attack</small></button><button data-move="tap">Gentle Bump<small>8 damage · useful before capture</small></button><button data-move="guard" ${!b.heals?'disabled':''}>Guard & Recover<small>Heal 14 HP · block 60% · ${b.heals} left</small></button><button data-move="catch" ${b.trainer||!state.cubes?'disabled':''}>Throw Capture Cube<small>${b.trainer?'Trainer companions cannot be caught':`${state.cubes} left · ${chance}% catch chance`}</small></button></div><button id="type-guide">◎ Type matchups</button><div class="switches">${state.party.map((c,i)=>i!==state.active&&c.hp?`<button data-switch="${i}">Switch to ${name(c)}</button>`:'').join('')}<button id="retreat">Retreat to grove</button></div><p class="battle-tip">${species[a.id].type} attacks are strong against ${effective.join(', ')||'no special type'}. Capture is guaranteed at 35% HP or less. Switching uses a turn.</p>`,'battle');
...
function roundWon(defeated){
  ...
  panel(`<span class="eyebrow">BOSS · ${b.round} OF ${bossTeam.length} DEFEATED</span><h1>${defeated} defeated!</h1><p>Next up: ${name(b.enemy)} (${species[b.enemy.id].type}). Your team is healed and recoveries are refilled for the next round.</p><canvas class="next-opponent" data-id="${b.enemy.id}" aria-label="${name(b.enemy)}"></canvas><button id="next-round">Battle ${name(b.enemy)}</button>`,'result');
...
async function turn(move,index){
  ...
  if(move==='catch'){
    ...
    const t=talents[species[a.id].type];sayBattle(`${name(a)} uses ${t.name}!`);await strike('player',species[a.id].type,typeFactor(species[a.id].type,species[b.enemy.id].type));const result=talentHit(a,b.enemy,b.enemyShield);b.enemyShield=false;b.talentUses[state.active]=(b.talentUses[state.active]||0)+1;talentGuard=result.guard;log=`${t.name}: ${result.damage} damage. ${matchupText(typeFactor(species[a.id].type,species[b.enemy.id].type))} `;showHP();
  }else{
    sayBattle(`${name(a)} uses ${move==='skill'?species[a.id].move:'Gentle Bump'}!`);await strike('player',move==='skill'?species[a.id].type:'stone',move==='skill'?typeFactor(species[a.id].type,species[b.enemy.id].type):1);const n=attack(a,b.enemy,move,b.enemyShield);b.enemyShield=false;log=`${name(a)} dealt ${n} damage. ${matchupText(move==='skill'?typeFactor(species[a.id].type,species[b.enemy.id].type):1)} `;showHP();
  }
  if(b.enemy.hp<=0){
    const defeated=name(b.enemy);gainXP(a,20);if(['skill','talent'].includes(move)&&typeFactor(species[a.id].type,species[b.enemy.id].type)>1)state.effectiveWins++; 
  ...
  if(['attack','burst'].includes(intent.kind))await strike('enemy',species[b.enemy.id].type,typeFactor(species[b.enemy.id].type,species[a.id].type));
  else await bossSpecialEffect(intent.kind);
  const result=resolveBossAction(b,a,move==='guard'||talentGuard);showHP();log+=`${intent.name}: ${result.damage?result.damage+' damage. '+matchupText(typeFactor(species[b.enemy.id].type,species[a.id].type)):intent.detail}`;b.turn++;
  if(!a.hp){
...
addEventListener('keydown',e=>{if(e.key!=='Tab'||!overlay.children.length)return;const list=[...overlay.querySelectorAll('button:not(:disabled)')];if(!list.length)return;const first=list[0],end=list.at(-1);if(e.shiftKey&&document.activeElement===first){end.focus();e.preventDefault();}else if(!e.shiftKey&&document.activeElement===end){first.focus();e.preventDefault();}});
...
function frame(now){
  ...
  if(state.party.length){r.person(player.x,player.z);r.creature(state.party[state.active].id,player.x-.65,.02,player.z+.55,.65,{rare:state.party[state.active].rare,stage:state.party[state.active].stage,heading:player.heading,moving:player.moving,runCycle:player.runCycle});}
  r.flush();
  ...
  for(const c of wild)if(now>c.cooldown&&cue?.target!==c)label(`${c.rare?'✦ Rare ':''}${c.name} · ${species[c.id].type}`,c.x,c.z,1.75);
...
function matchupText(f){return f>1?'Super effective! ×1.5 damage.':f<1?'Resisted! ×0.75 damage.':'Normal hit. ×1 damage.';}
function showTypeGuide(){
  ...
  panel(`<span class="eyebrow">TYPE SCHOOL</span><h1>Find the strong matchup</h1><p>Read from the attacking type to its target. ✦ Strong = 1.5× damage. ◇ Resisted = 0.75× damage. All other matchups = 1×. Gentle Bump always ignores types.</p>${previous==='battle'?`<p class="matchup-summary">Your ${species[a.id].type} → their ${species[b.enemy.id].type}: ${matchupText(typeFactor(species[a.id].type,species[b.enemy.id].type))}<br>Their ${species[b.enemy.id].type} → your ${species[a.id].type}: ${matchupText(typeFactor(species[b.enemy.id].type,species[a.id].type))}</p>`:''}<div class="type-table"><table><thead><tr><th>Attack type</th><th>✦ Strong against</th><th>◇ Resisted by</th></tr></thead><tbody>${Object.keys(strengths).map(t=>`<tr><th>${t}</th><td>${strengths[t].join(', ')}</td><td>${Object.keys(strengths).filter(d=>typeFactor(t,d)<1).join(', ')||'—'}</td></tr>`).join('')}</tbody></table></div><button id="close-types">${previous==='battle'?'Back to battle':'Back to grove'}</button>`);
...
function showMap(){journalTab('Choose your next destination',`<p>Walk freely across the world, or travel to a region's trailhead. Each region has its own creatures and a treasure chest. Bosses wait north of Home Grove.</p><div class="region-grid">${regions.map(r=>`<button data-travel="${r.id}">${r.name}<small>${r.species.map(id=>species[id].type).join(' · ')}${state.visited.includes(r.id)?' · Visited':''}</small></button>`).join('')}</div>`);overlay.querySelectorAll('[data-travel]').forEach(b=>b.onclick=()=>{const r=regions.find(r=>r.id===b.dataset.travel);player.x=r.x;player.z=r.z;camera.x=r.x;camera.z=r.z;closePanel();state.safeUntil=performance.now()+2200;toast(`Welcome to ${r.name}! Look for the gold treasure chest.`);});}
$('#map').onclick=()=>{if(state.mode==='explore')showMap();};

function showJournal(){
 const cards=collectibleIds.map(id=>{const owned=state.party.filter(c=>c.id===id),seen=state.seen.includes(id);return `<article class="dex-card"><h3>${seen?species[id].name:'Unknown companion'}</h3>${seen?`<canvas data-id="${id}" aria-label="${species[id].name}"></canvas><p>${species[id].type} · ${owned.length?'Caught':'Seen, not caught'}</p><small>Forms: ${[0,1,2].map(n=>state.forms.some(f=>f.startsWith(id+':'+n))?['Base','Super','Royal'][n]:'?').join(' → ')}<br>Rare: ${owned.some(c=>c.rare)?'✦ Collected':'Not found'}</small>`:'<p>Explore other regions to discover it.</p>'}</article>`;}).join('');
 journalTab('Quests & companion collection',`<p>Quest rewards: 3 cubes and 40 XP for every companion. Rare gold-and-violet creatures can be found in the wild; collect them alongside their normal form.</p><div class="quest-list">${quests.map(q=>`<article><strong>${q.name}</strong><p>${q.detail} ${Math.min(q.goal,q.value(state))}/${q.goal}</p><button data-claim="${q.id}" ${state.claimed.includes(q.id)||q.value(state)<q.goal?'disabled':''}>${state.claimed.includes(q.id)?'Claimed':'Claim reward'}</button></article>`).join('')}</div><h2>Companion collection</h2><div class="region-grid">${cards}</div>`);
