...
function evolveCompanion(i){
  ...
  state.mode='evolution';updateHUD();panel(`<span class="eyebrow">EVOLUTION!</span><h1>Meet ${name(c)}!</h1><canvas class="next-opponent evolution-portrait" data-id="${c.id}" data-stage="${c.stage}" data-rare="${!!c.rare}"></canvas><p>${before} evolved! +18 maximum HP, +${c.stage===3?105:5} skill power. ${c.stage===3?"Mega power is strongest against neutral or weak targets. Resistance and immunity still apply!":""} ${c.stage===1?"New skill unlocked":"Skill upgraded"}: ${talents[species[c.id].type].name} · ${c.stage+1} uses per encounter! ${talents[species[c.id].type].detail} Fully healed.</p><button id="finish-evolution">Keep adventuring</button>`,'result');$('#finish-evolution').onclick=closePanel;
}
...
function starters(){
  ...
  overlay.querySelectorAll('[data-starter]').forEach(b=>b.onclick=()=>{state.party=[makeCreature(b.dataset.starter)];closePanel();toast('Touch a wild creature to battle! Pufflet is west of camp — a friendly first capture.');});
}
...
async function turn(move,index){
  ...
  if(move==='catch'){
    ...
    const t=talents[species[a.id].type];sayBattle(`${name(a)} uses ${t.name}!`);await strike('player',attackType(a,b.enemy),matchup(a,b.enemy),a.stage===3);const result=talentHit(a,b.enemy,b.enemyShield);b.enemyShield=false;b.talentUses[state.active]=(b.talentUses[state.active]||0)+1;talentGuard=result.guard;log=`${t.name}: ${result.damage} damage. ${matchupText(matchup(a,b.enemy))} `;showHP();
  }else{
    sayBattle(`${name(a)} uses ${move==='skill'?species[a.id].move:'Gentle Bump'}!`);await strike('player',move==='skill'?attackType(a,b.enemy):'rock',move==='skill'?matchup(a,b.enemy):1,a.stage===3&&move==='skill');const n=attack(a,b.enemy,move,b.enemyShield);b.enemyShield=false;log=`${name(a)} dealt ${n} damage. ${matchupText(move==='skill'?matchup(a,b.enemy):1)} `;showHP();
  }
  if(b.enemy.hp<=0){
  ...
  if(['attack','burst'].includes(intent.kind))await strike('enemy',attackType(b.enemy,a),matchup(b.enemy,a));
  else await bossSpecialEffect(intent.kind);
  const result=resolveBossAction(b,a,move==='guard'||talentGuard);showHP();log+=`${intent.name}: ${['attack','burst'].includes(intent.kind)?result.damage+' damage. '+matchupText(matchup(b.enemy,a)):intent.detail}`;b.turn++;
  if(!a.hp){
...
function matchupText(f){return f===0?'Immune! No damage. ×0.':f>1?`Super effective! ×${f} damage.`:f<1?`Resisted! ×${f} damage.`:'Normal hit. ×1 damage.';}
function showTypeGuide(){
  ...
  const a=state.party[state.active],b=state.battle,types=Object.keys(typeIcons),options=types.map(t=>`<option value="${t}">${typeText(t)}</option>`).join('');
  panel(`<span class="eyebrow">TYPE SCHOOL · ALL 18 TYPES</span><h1>Learn the type matchups</h1><p>✦ Strong: ×2 · ◇ Resisted: ×0.5 · ○ Immune: ×0 · Neutral: ×1. For two defensive types, multiply both factors: ×4 and ×0.25 are possible, and immunity always wins.</p><p>Grass replaces the old Leaf label; Rock replaces Stone. Ground is a different type. In this game, skills automatically choose the better of your companion's two attacking types. Gentle Bump is a special capture helper that ignores types.</p><div class="dex-filters"><label>Attack type<select id="chart-attack">${options}</select></label><label>Defending type<select id="chart-defense">${options}</select></label><label>Second defending type<select id="chart-second"><option value="">None</option>${options}</select></label></div><div id="chart-result" class="matchup-summary" role="status" aria-live="polite"></div>${previous==='battle'?`<p class="matchup-summary">Your ${typeLabel(a)} → their ${typeLabel(b.enemy)}: ${matchupText(matchup(a,b.enemy))}<br>Their ${typeLabel(b.enemy)} → your ${typeLabel(a)}: ${matchupText(matchup(b.enemy,a))}</p>`:''}<details><summary>Full chart — all 18 attacking types</summary><div class="type-table"><table><thead><tr><th>Attack</th><th>✦ ×2 against</th><th>◇ ×0.5 against</th><th>○ ×0 against</th></tr></thead><tbody>${types.map(t=>`<tr><th>${typeText(t)}</th>${[2,.5,0].map(f=>`<td>${types.filter(d=>typeFactor(t,d)===f).map(typeText).join(', ')||'—'}</td>`).join('')}</tr>`).join('')}</tbody></table></div></details><p class="muted">Type chart: <a href="https://diamondpearl.pokemon.com/en-gb/trainersguide/fundamentals/battling/" target="_blank" rel="noopener noreferrer">Official Pokémon Trainers Guide</a>. This game uses its own moves and damage stats.</p><button id="close-types">${previous==='battle'?'Back to battle':'Back to grove'}</button>`);
  $('#chart-attack').value=creatureTypes(a)[0];$('#chart-defense').value=previous==='battle'?creatureTypes(b.enemy)[0]:'grass';$('#chart-second').value=previous==='battle'?(creatureTypes(b.enemy)[1]||''):'';
  const calculate=()=>{const t=$('#chart-attack').value,d=$('#chart-defense').value,s=$('#chart-second').value,f=typeFactor(t,d),second=s&&s!==d?typeFactor(t,s):1;$('#chart-result').textContent=`${typeText(t)} → ${typeText(d)}${s&&s!==d?' + '+typeText(s):''}: ${matchupText(f*second)}${s&&s!==d?' ('+f+' × '+second+')':''}`;};
  for(const id of ['chart-attack','chart-defense','chart-second'])$('#'+id).onchange=calculate;calculate();
