import {normalizeStory} from './story.js?v=story2';
import {species,makeCreature,collectibleIds} from './rules.js?v=story2';
...
export function decodeSave(raw){
 ...
 return {story:normalizeStory(d.story,list(d.visited)),party,active:num(d.active,0,party.length-1),cubes:num(d.cubes,5,99),wins:num(d.wins),bossIndex:num(d.bossIndex),badge:!!d.bossIndex,seen:list(d.seen),visited:list(d.visited),opened:list(d.opened),claimed:list(d.claimed),captures:num(d.captures),effectiveWins:num(d.effectiveWins),forms:list(d.forms),player:{x:position(d.player?.x),z:position(d.player?.z)}};
}
export function encodeSave(state,player){const {story,party,active,cubes,wins,bossIndex,seen,visited,opened,claimed,captures,effectiveWins,forms}=state;return JSON.stringify({version:1,story,party,active,cubes,wins,bossIndex,seen,visited,opened,claimed,captures,effectiveWins,forms,player:{x:player.x,z:player.z}});}
