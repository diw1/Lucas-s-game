import {species,makeCreature} from './rules.js';
...
export function decodeSave(raw){
 ...
 const party=d.party.slice(0,14).map(c=>{if(!species[c.id])throw Error('Unknown companion');const stage=num(c.stage,0,2),max=species[c.id].max+stage*18;return {...makeCreature(c.id),stage,max,hp:num(c.hp,max,max),xp:num(c.xp),rare:c.rare===true};});
...
export function writeSave(storage,state,player){try{storage.setItem(SAVE_KEY,encodeSave(state,player));return true;}catch{return false;}}
